...
> **推荐阅读：**
>
> - **我想几条命令跑起来**： [快速开始](#快速开始) 用 **uv**（推荐）→ 浏览器打开控制台。
...
先安装 [uv](https://docs.astral.sh/uv/getting-started/installation/)（独立安装脚本、Homebrew、`pip install uv` 等）。
...
| 后端          | 适用场景                          | 安装                                                                 |
| ------------- | --------------------------------- | -------------------------------------------------------------------- |
| **llama.cpp** | 跨平台（macOS / Linux / Windows） | `uv pip install 'copaw[llamacpp]'` / `pip install 'copaw[llamacpp]'` 或 `bash install.sh --extras llamacpp` |
| **MLX**       | Apple Silicon（M1/M2/M3/M4）      | `uv pip install 'copaw[mlx]'` / `pip install 'copaw[mlx]'` 或 `bash install.sh --extras mlx`         |
| **Ollama**    | 跨平台（需要 Ollama 服务运行）    | `uv pip install 'copaw[ollama]'` / `pip install 'copaw[ollama]'` 或 `bash install.sh --extras ollama`   |
...
# Python：uv（推荐，使用仓库内 uv.lock）
uv sync
```

- **运行**：在仓库根目录执行 `uv run copaw init --defaults`，再 `uv run copaw app`（使用 `.venv`）。
- **开发**（测试、格式化、可选依赖）：`uv sync --extra dev --extra full`，或已在 venv 中时 `uv pip install -e ".[dev,full]"`。
- **不用 uv**：`pip install -e .`，再 `copaw init --defaults` 与 `copaw app`。

依赖变更后可在仓库根目录执行 `uv lock` 并提交更新后的 `uv.lock`。

### 本地开发（热更新）

在仓库根目录（需安装 [Node.js](https://nodejs.org/)、[pnpm](https://pnpm.io/installation) 与 [uv](https://docs.astral.sh/uv/getting-started/installation/)）：

```bash
pnpm run setup                 # 根目录与 console 依赖 + uv sync --extra dev（并行）
uv run copaw init --defaults   # 每个克隆/环境执行一次
pnpm run dev
```

- **`pnpm run dev`** 只启动后端与 `vite build --mode embed --watch`（不再每次先跑一遍完整 `build:embed`，避免与 watch 首次构建重复）；watch 启动时会产出首包到 `src/copaw/console/`。若需要先执行 `tsc -b` 并做一次完整 embed 构建（例如大改前端后、或希望类型错误尽早暴露），请用 **`pnpm run dev:full`**。
- **agent**：`copaw app` 监听 **http://127.0.0.1:8088**，带 `--reload`。
- **console**：执行 `vite build --mode embed --watch`，直接输出到 **`src/copaw/console/`**（等价于手动 build 再拷贝）。请在浏览器打开 **http://127.0.0.1:8088/**；保存源码后等 watch 构建结束再刷新。若需要 Vite 开发服务器与 5173 端口的 HMR，可在另一终端单独执行 `pnpm -C console dev`（见 `console/vite.config.ts` 中的代理）。

若需与发布形态一致、**不做 watch** 的单进程运行，请按上文 [从源码安装](#从源码安装) 构建并复制 `console/dist` 后执行 `uv run copaw app`，浏览器访问 **http://127.0.0.1:8088/**。
